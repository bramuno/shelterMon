...
<li>The equipemnt required for this project only produces a max of 5vdc.  This is less than 9v batter and it's technically safe to put that directly on your toungue (just an example, not recommrnding it.   Onec the USB power adapter ( aka cell phone charger ) converts the electricity into lower voltage current, any danger of electrocultion is removed.  HOWEVER, it is not impossible depending on your exact setup. <br>For example, if you were to use an extention cord to bring the usb power adapter to a different place in the room as needed, it would technically be safe for the anmimals to chew on the USB cable or ESP device and not get electrocuted.  BUT if they were to chew on the extension cord itself, that could easily kill an animal or human as that is unconverted high voltage current.   It is strongly recommended  that you consult with an electrician if you don't know anything about electrical safety.  </li>
</ul>
